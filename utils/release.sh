...
# Github-project:
gh_project=specterext-stacktrack
# defining the remote
remote=git@github.com:cryptoadvance/${gh_project}.git
...
function main() {
    ...
    if ! [ "$(git remote -v | grep upstream | grep $remote |  wc -l | sed -e 's/\s*//')" = "2" ]; then
        echo "    --> You don't have the correct upstream-remote. You need this to release. Please do this:"
        echo "git remote add upstream $remote "
        exit 2
    fi
